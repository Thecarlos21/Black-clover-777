...
export async function blackJadiBot(options) {
  ...
  exec(comb.toString("utf-8"), async () => {
    ...
    async function requestPairingCode() {
      if (!mcode || pairingRequested) return

      if (state.creds.registered) return

      try {
        const phoneNumber = String(m.sender || '')
          .split('@')[0]
          .replace(/\D/g, '')
        ...
        if (!phoneNumber || phoneNumber.length < 7) {
          await conn.reply(
            m.chat,
            '⚠️ No se pudo obtener correctamente tu número de WhatsApp para generar el código.',
            m
          )
          return
        }
    ...
    async function autoSnapshot() {
      try {
        ...
        if (fs.existsSync(pathCreds)) {
          fs.copyFileSync(
            pathCreds,
            path.join(snapDir, `snapshot_${timestamp}.json`)
          )
    ...
    function startKeepAlive() {
      ...
      keepAliveInterval = setInterval(() => {
        try {
          if (
            sock?.ws?.socket?.readyState === ws.OPEN &&
            sock?.ws?.socket?.ping
          ) {
            sock.ws.socket.ping()
    ...
    function startWatchdog() {
      ...
      watchdogInterval = setInterval(() => {
        try {
          ...
          if (
            !sock?.ws?.socket ||
            sock.ws.socket.readyState === ws.CLOSED ||
            sock.ws.socket.readyState === ws.CLOSING
          ) {
            creloadHandler(true).catch(() => {})
          }
        } catch {}
    ...
    async function connectionUpdate(update) {
      ...
      if (connection === 'close') {
        ...
        const delay = Math.min(
          3000 * Math.pow(2, reconnectAttempts) + jitter,
          maxReconnectDelay
        )
      ...
      sock.ev.on(
        ...
        async msg => {
          try {
            ...
            const config =
              global.subBotConfig?.get(sender)
