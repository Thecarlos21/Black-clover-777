...
var handler = async (m, { conn }) => {
    ...
    let chatList = conn.chats || {}

    let chats = Object.entries(chatList)
        .filter(([id, data]) => id && data && data.isChats)

    let groups = Object.entries(chatList)
        .filter(([jid, chat]) =>
            jid &&
            jid.endsWith('@g.us') &&
            chat &&
    ...
    let subBots = global.conns?.filter(c =>
        c &&
        c.user &&
        c.ws?.socket?.readyState === 1
    ).length || 0
